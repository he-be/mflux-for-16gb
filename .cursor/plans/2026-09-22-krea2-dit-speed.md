...
### M8a. 活性を bf16 にする ← 済(**合格**: 13.08 s/step、画像は機材差より小さい差)
...
### M8b. I/O を計算と重ねる ← 済(**合格**: 9.42 s/step、+0.46 GB、出力は完全一致)
...
### M8c. `mlp.down` を K で 4 分割する ← 済(**不採用**: 単体 −30 ms だが本番はプリフェッチと干渉して +42 ms)
...
### M8d. `mx.clear_cache()` の回数を減らす ← 済(**採用**: −12 ms、footprint は 1280² で 6.92 → 6.07 GB)
...
### M8e. 持続負荷でクロックが落ちるかを測る ← 代替計測のみ(1 枚生成では落ちない。`powermetrics` は sudo が要るので未実施)
...
### M8f. M3 Pro 18GB で回帰 ← 済(**合格**: 29.6 → 22.96 s/step、footprint 4.66 GB)
...
## 3. 到達点(1024²、M6 mini、1 ステップ)— 見込みと実測

| 段 | 計算 / ブロック | 見える I/O | 見込み | **実測** |
|---|---|---|---|---|
| M7(出発点) | 610 ms | 140 ms | 21.2 s | 21.2 s |
| M8a | 320 ms | 140 ms | 13.1 s | **13.08 s** |
| + M8b | 320 ms | 0 | 9.2 s | **9.42 s** |
| + M8d(M8c は不採用) | ≈ 323 ms | 0 | 8.9 s | **9.26 s** |
| 理論下限 | 240 ms | 0 | 6.9 s | — |

1280²: 35.2 → **15.88 s**。M3 Pro 1024²: 29.6 → **22.96 s**。
