...
- 状態: **M11a-3 で測り直し(2026-09-23)。M11a の不合格は `mlp57`(down 込み)という同居人の話で、
  gate/up だけを ANE に出す形なら GPU+ANE の合計は単独の 1.4 倍、本番の横でも +28% で済む。**
  **M11b-lite(block 0 実物、`tools/bench/ane_block.py`)は 1 ブロック −4〜−5%。実装を理想化しても −6〜−8%。**
  合成の 1.4 倍が消えるのは、ANE の窓が MLP の gate/up の間だけ(ブロックの 3 分の 1)で、ANE の答えを使う down が直列に残るから。
  行方向のパイプラインも 2 プロセスも効かない。代償は常駐 +4 GB、worker プロセス、gate/up の再量子化。
  **採るかどうかは持ち主の判断**(M11a-3 §7)。合否線はもう引かない。
  測定: [M11a](../../docs/16gb/measurements/2026-09-22-m11a-ane-probe.md)、
  [M11a-3: 併走の損はどこから来るか](../../docs/16gb/measurements/2026-09-23-m11a3-ane-coscan.md)
...
**結果(2026-09-23 確定、`docs/16gb/measurements/2026-09-22-m11a-ane-probe.md`)**:

判定は **本番の生成の横で ANE に候補の形を回させて** 決めた(初版は §4-3 の指定どおり合成の行列積連打で
決めていた。**その指定が計画の誤り**で、本番で取り直した)。

| # | 基準 | 実測 | |
|---|---|---|---|
| 1 | 併走下で 15 TOPS 以上 | 9.2〜9.3(単独 15.4) | ✗ |
| 2 | GPU の低下 5% 以内 | **本番 1 ステップ 7.84 → 18.90〜19.11 s(+133〜143%、3 回再現)** | ✗ |
| 3 | 受け渡し 5 ms 以内 | 3.5 ms | ✓ |
| 4 | 常駐が見積りの ±20% | 約 5.7 GB(+12%) | ✓ |
| 5 | 出力差が norm の bf16 化と同程度 | **未判定**(§4-7 のとおり画像で判定すべきところ、速度で落ちたので撮っていない) | — |

**増えた時間は全部 compute**(1 ブロック 335 → 724 ms、2.16 倍)。**prefetch は 140.4 ms のまま不動**、
swapouts 0、pagein も不変。分割にすると GPU 側は 250 ms × 2.16 = 543 ms/block で
**1 ステップ 9.4 → 15.2 s(1.6 倍遅い)**、常駐 +5.7 GB。

**機構は未特定。** 電力・クロック調停か記憶階層の競合か。干渉が ANE の演算量ではなく作業セットに追随する
(38 MB → +37%、172 MB → +133%)ことだけが分かっている。`powermetrics`(要 sudo)で切れる。

**計画の誤り 2 つ**(同じ轍を踏まないため):
- **§4-3 が合成の同居人を指定していた。** 本番の可否を本番以外で決める設計になっていた。
- 初版の測定文書は「帯域律速だから」という**測っていない機構**を結論の理由に書いた。検算すると
  GPU 6.0 GB/s + ANE 5.5 GB/s で、バス(≥120 GB/s)の 1 割も使っていない。撤回済み。

**M11a-3(2026-09-23)での読み直し**(`docs/16gb/measurements/2026-09-23-m11a3-ane-coscan.md`):
同居人の中身と GPU カーネルを振ると、**損の 4 割は down(K の大きい行列積)を ANE に載せたことに由来**し、
gate/up + silu·mul だけ(`gateup57`)なら GPU −20%、ANE 10.4 TOPS で **合計 1.41 倍**、本番の横でも **+28%**(`mlp57` は +133%)。
残りの 2 割は ANE 固有ではなく、同居人がメモリ系に出すトラフィックの「基本料」(演算なしの DMA でも −11%、CPU で回しても −16%)。
クロックの落ち(0.85)は速度の落ち(0.46)を説明せず、sdpa は同居人に平気なので、**クロックが主因ではない**。
**合格基準 1 / 2 は「同じ 1 つの仕事を ANE に丸ごと出しても GPU が減速しない」を要求していて、演算器を足す目的と噛み合っていなかった**
(基準 2 の「5% 以内」は、合計が増えるかどうかを見ていない)。以後は「価格 = GPU が失う TFLOPS ÷ ANE が出す TOPS」で読む。

### M11b-lite. 1 ブロック実物で分割を測る — **済(2026-09-23、M11a-3 §6)**

block 0 の実重みで、**gate/up を ANE、down は GPU** の分割を組んで 1 ブロックの時間を単独と比べた(`tools/bench/ane_block.py`)。

| | 1 ブロック(alone4 = 276 ms) |
|---|---|
| fp16 活性、a = 0.57 | **−3.7%** |
| int8 活性(a8)、a = 0.57 | **−4.9%**(数値は画像判定まで採用しない) |
| 行 2 分割のパイプライン | 効かない(+1.8% / −0.3%) |
| a を釣り合い(0.45〜0.5)に寄せた理想の実装 | **−6〜−8%** |

見積りの −8% は「理想の実装」でしか出ない。1 ステップに直すと 9.4 → 9.0 s(実測)〜 8.7 s(理想)。

### M11b. MLP の列分割を本番に — **持ち主の判断待ち**(down は GPU に残す形に変更)
