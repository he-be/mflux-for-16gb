...
| 失敗 | 兆候 | 撤退先 |
|---|---|---|
| I/O が隠れない | M1 で比 < 2.0 | ストリーミングを捨て、段階ロード(TE→DiT→VAE)に戻す。**q4 のみ**(下記) |
...
### 撤退先の数字と、wired_limit について

このマシンの実測値: `hw.memsize` 19.33GB、`iogpu.wired_limit_mb` 14336MB (14.34GB)、
`max_recommended_working_set_size` 15.03GB、`mx.set_wired_limit` の既定は 0
(MLX は何も wire していない)。

| 構成 | 段階ロードのピーク(重み) | 判断 |
|---|---|---|
| **q4 段階ロード = 撤退先** | max(TE 8.05, DiT 7.21 + LoRA 0.44) = **8.05GB** | ピークは TE 側。DiT は 7.65GB で余裕あり |
| q8 段階ロード | max(8.05, 14.06 + 0.44) = **14.50GB** | システムの wired 上限 14.34GB を超える。**採らない** |

wired_limit は確保上限ではなく「常駐を保証する量」。上げなくても 14.5GB の確保自体は
できるが、そのページは圧縮・スワップの対象になる。逆に 14.5GB を wire すれば OS と
アクティベーションが残り 4.8GB を奪い合う。**sysctl を書き換えてギリギリ載せる方針は
採らない。** 撤退先は q4 で、これは wired_limit を触らずに成立する。

ただし逆向きの使い方は検討に値する: ストリーミング中はページキャッシュが激しく回るので、
常駐させたい小物(globals、LoRA、アクティベーション)の 2〜3GB だけを `mx.set_wired_limit`
で押さえ、追い出されないようにする。必要性は M2 で測ってから判断する。
