...
- **不合格なら**: 「18GB 機に q8 DiT は常駐できない」を数字つきで結論とし、M5 へ進む。
  q4 に落とす選択肢は取らない。

### M4. VAE をプロセス 3 でデコード

- latent を読んで画像にする。常駐 0.51 GB + タイルデコード。
- ここまでで **q8 + 4step の実画像が 1 枚出る**。出力・CSV・所要時間を
  `docs/16gb/runs/` に記録。

### M5. ブロック単位ストリーミング(M3 の結果にかかわらず測る)
...
**合格基準**: プロセス分割版(M2〜M4)と同じ数字が mflux 経由でも出ること。
