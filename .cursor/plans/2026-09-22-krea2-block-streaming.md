...
- **結果: 合格。** 実装は `d9b2f80`。[記録](../../docs/16gb/measurements/2026-09-22-m6-mflux-cli.md)

  | 基準 | 実測 | |
  |---|---|---|
  | 29.8 s/step | **29.60 s** | ✅ |
  | 比 13.65 | **13.59**(I/O 71.9 ms、計算 976.9 ms) | ✅ |
  | peak 3.72 GB | DiT 段はその通り。全体のピークは TE の 4.47 GB / footprint 5.21 GB | ✅ |
  | `tools/bench/` 版とバイト一致 | **ピクセル完全一致**(1048576 中 0 ピクセル違い)。PNG のバイトは EXIF の分だけ違う | ✅ |
  | swapouts 0 | 1280² で **0**。1024² は 56 ページだが外来(下) | ✅ |

  **56 ページの件**: 1024² の実行で swapwatch が `SWAPPED` を出したが、中身は 1 サンプル
  だけの 56 ページ = 0.9 MB で、同じサンプルで swap used が 21 MB **減り** swapins が 81
  増えていた。他プロセスの読み戻しの裏返しで、自プロセスの footprint は動いていない。
  swapout カウンタはマシン全体なので、この規模は区別できない。
  **より重い 1280²(footprint 6.89 GB)が 208 秒で swapouts 0 だったことで決着した。**

  **まだ残っていること**: この 2 本は前セッションが残した `iogpu.wired_limit_mb=15360`
  の下で測っている。ストリーミングにこの設定は不要なので、**0 に戻して測り直す**。
...
- q8 + 4step LoRA、1024²、seed 42、swapwatch 下。2 回実行して画像一致。
  → **実質済み。** M6 の CLI 出力が、40 分前の別プロセス(`tools/bench/`)の結果と
  ピクセル完全一致した。
- その後 1280²(LoRA の学習 σ と一致する解像度)。→ **済。clean。**
  48.91 s/step、footprint 6.89 GB、swapouts 0、計算 ÷ I/O 22.92。
  1024² より明らかに描き込みが多い(LoRA の学習点 μ=1.15 と一致する解像度)。
  `docs/16gb/runs/images/20260922-1129-m6-cli-1280.png`
