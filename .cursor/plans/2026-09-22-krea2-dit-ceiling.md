...
### M9b. 323 ms の内訳を 1 つのプロセスで確定する(1 時間)
...
### M9c. `mlp.down` の崖を消す(本命、−1.1〜1.3 s/step)
...
### M9e. プリフェッチの汚染(1280² で −0.9 s/step、1024² は −0.1)
...
### M9g. M3 Pro で回帰 + `iogpu.wired_limit_mb=0` 再測定(既存の宿題)
...
| 段 | ブロック | 1 ステップ | 対いま |
|---|---|---|---|
| いま(M8d) | 323 ms | **9.26 s** | 1.00 |
| + M9c(崖) | ≈ 280 | **≈ 8.0 s** | 1.16 |
| + M9d(融合) | ≈ 260 | **≈ 7.5 s** | 1.24 |
| 天井(MLX が MPS に追いつく) | 213 | ≈ 6.2 s | 1.49 |

1280²: 15.88 s → M9c で約 13.5、M9e が効けば約 12.6。
生成全体(TE 2 s + DiT + VAE 6 s)は 1024² で 45 → 38 s 前後。
