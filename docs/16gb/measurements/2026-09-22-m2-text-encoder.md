...
- 測定条件: 2 通りで測った。(a) 再起動前(常駐アプリあり、claimable 7.5〜11.0 GB、
  swap 既使用 492 MB)3 回、(b) [M1 の再起動後](2026-09-22-m1-clean-baseline.md)
  (swap 0、compressor 0)1 回。**数字は同じ**だった。

## 1. 判定

**合格。`clean`。**

同じ実行を 4 回(汚れた状態 3 回 + クリーンな状態 1 回)。数字は 4 回とも一致した。
...
実体化はクリーンな状態では 2.14 s(3.8 GB/s)に伸びた。5.9 GB/s は M0 の実測帯域(全体読みで 4.47 GB/s、拾い読みで 6.24 GB/s)と
...
- **compressor が伸びる。** 汚れた状態では 1.26 → 6.21 GB、クリーンな状態では
  **0.00 → 8.88 GB**。swapouts は 0 なので判定は `clean` だが、8 GB の場所は
  「他のプロセスを圧縮して」作られている。**クリーンでも余裕があるわけではない**ことが
  ここで既に見えていた(M3 の段 1 はこの延長で落ちた)。
...
TE (8.05 GB) と DiT (13.62 GB) を別プロセスにすれば、**TE 側は 18GB 機で通る**。
