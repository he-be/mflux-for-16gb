...
- ~~**M11a: ANE を 2 台目の演算器として足せるかの probe**~~ → **済。不合格で閉じた**
  ([M11a](measurements/2026-09-22-m11a-ane-probe.md))。**本番の生成の横で ANE に候補の形を回させると
  7.84 → 18.90〜19.11 s/step(+133〜143%、3 回再現)**、そのとき ANE が出せたのは 9.3 TOPS(単独 15.4)。
  **増えた時間は全部 compute**(335 → 724 ms/block)で、**prefetch は 140.4 ms のまま不動**、swapouts 0、
  pagein も不変。分割にすると 9.4 → 15.2 s/step で常駐 +5.7 GB。**機構は未特定**(電力か記憶階層か、
  `powermetrics` 待ち)。付随: デュアル ANE は 1 デバイスで 2 プロセス +34%、Core ML は group-64 int8 を
  ANE に載せない、fp16 活性のレンジは余裕 33 倍。**M11b / M11c には進まない。**
- **手順の教訓**: M11a の初版は合成の行列積連打(計画 §4-3 の指定)で本番の可否を決め、さらに測っていない
  機構(「帯域律速」)を結論の理由に書いていた。**本番の形で測り直すまで判定しないこと。**
